...
//在标签vector中更新标签{key, value}(不存在则插入)
void insert_sorted_two_hop_label(std::vector<two_hop_label_v1>& input_vector, int key, weightTYPE value) {
	...
	xx.vertex = key;
	xx.distance = value;
	while (left <= right) // it will be skipt when input_vector.size() == 0
	{
...
//在标签vector中查找key顶点对应的{distance, 编号}(不存在则返回{DOUBLE_MAX,-1})
...
//遍历L(s),L(t)寻找交集, 返回s,t之间的{最短距离,hub编号}
