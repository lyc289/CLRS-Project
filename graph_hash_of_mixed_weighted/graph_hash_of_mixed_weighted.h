...
class graph_hash_of_mixed_weighted_vectors {//无向图, 点和边都有权重
public:
...
public:
	std::unordered_map<int, graph_hash_of_mixed_weighted_vectors> hash_of_vectors; // hash of vectors for all vertices, 存储整张图的点, 哈希表采用unordered_map实现, {vertex, {v_weight, {adj_v, edge_weight}}}
	std::unordered_map<int, std::unordered_map<int, double>> hash_of_hashs; // hash of hashs for some vertices (adjacenct vertices and weights of edges), 存储部分点(类似cache), {vertex, {adj_vertex, edge_weight}}
	void clear() {
