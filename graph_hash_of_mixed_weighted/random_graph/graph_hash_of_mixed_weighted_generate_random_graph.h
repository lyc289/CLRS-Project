...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_generate_random_graph(long long int V, long long int E, double nw_min, double nw_max,
	...
	//生成V个权值随机的顶点
	graph_hash_of_mixed_weighted random_graph; 
	for (int i = 0; i < V; i++) {
		double new_weight = (double)dist_nw(boost_random_time_seed) / precision; 
		graph_hash_of_mixed_weighted_add_vertex(random_graph, i, new_weight);
	}

	/*add edges to random_graph*/
	long long int max_E = V * (V - 1) / 2; 
	if (E == max_E) { // 完全图
		for (int i = 0; i < V; i++) {
			for (int j = 0; j < i; j++) {
		...
		std::vector<int> not_full_vertices; // vertices without a full degree初始化为所有顶点
		for (int i = 0; i < V; i++) {
		...
		while (edge_num < E) {
			...
			if (graph_hash_of_mixed_weighted_degree(random_graph, not_full_vertices[RAND]) < V - 1) { // 随机选一个度不满的顶点
