...
//将graph_hash_of_mixed_weighted转换为vector<vector<pair<int, double>>>
graph_v_of_v_idealID graph_hash_of_mixed_weighted_to_graph_v_of_v_idealID_2(graph_hash_of_mixed_weighted& input_graph, int max_N) {
	...
	for (auto it1 = input_graph.hash_of_vectors.begin(); it1 != input_graph.hash_of_vectors.end(); it1++) {//{vertex, {v_weight, {adj_v, edge_weight}}}
		int i = it1->first;
